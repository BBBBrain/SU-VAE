...
import numpy as np
from SU_VAE2_charm import CVAE
from SU_VAE2_charm import CVAE2
from SU_VAE2_charm import CVAE3
from SU_VAE2_charm import CVAE4
...
train_i1_dir = './data_charm/i1_train'
train_i2_dir = './data_charm/i2_train'
...
val_i1_dir = './data_charm/i1_eval'
val_i2_dir = './data_charm/i2_eval'
...
train_i3_dir = './data_charm/i1_train'
train_i4_dir = './data_charm/i2_train'
...
val_i3_dir = './data_charm/i1_eval'
val_i4_dir = './data_charm/i2_eval'
...
train_i5_dir = './data_charm/i1_train'
train_i6_dir = './data_charm/i2_train'
...
val_i5_dir = './data_charm/i1_eval'
val_i6_dir = './data_charm/i2_eval'
...
train_i7_dir = './data_charm/i1_train'
train_i8_dir = './data_charm/i2_train'
train_loader7 = ImageFolder(train_i7_dir).set_attrs(transform=transform1, batch_size=batch_size, shuffle=True)
train_loader8 = ImageFolder(train_i8_dir).set_attrs(transform=transform1, batch_size=batch_size, shuffle=True)
val_i7_dir = './data_charm/i1_eval'
val_i8_dir = './data_charm/i2_eval'
val_loader7 = ImageFolder(val_i7_dir).set_attrs(transform=transform1, batch_size=1, shuffle=False)
val_loader8 = ImageFolder(val_i8_dir).set_attrs(transform=transform1, batch_size=1, shuffle=False)
...
def loss3(out, inputs1,inputs2,  s1_out, s2_out):
    ...
    reconstruction_loss = nn.mse_loss(jt.flatten(outputs2), jt.flatten(inputs2))* 10*35*35*3  #15000
    reconstruction_loss += nn.mse_loss(jt.flatten(outputs1), jt.flatten(inputs1)) * 10*35*35*3

    reconstruction_loss += nn.mse_loss(jt.flatten(i1_s_output_), jt.flatten(s1_out)) * 10*35*35*3
    reconstruction_loss += nn.mse_loss(jt.flatten(i2_s_output_), jt.flatten(s2_out)) * 10*35*35*3
    ...
    reconstruction_loss += nn.mse_loss(jt.flatten(i1_z_output_), jt.flatten(zzzz) )* 10*35*35*3  #10000
    reconstruction_loss += nn.mse_loss(jt.flatten(i2_z_output_), jt.flatten(zzzz)) * 10*35*35*3
...
def loss4(out, inputs1,inputs2,  s1_out, s2_out):
    i1_z_mean, i1_z_log_var, i1_z = out[0][0], out[0][1], out[0][2]
    ...
    # reconstruction_loss += nn.mse_loss(jt.flatten(i1_s_output_), jt.flatten(s1_out)) * 10*23*23*3
    # reconstruction_loss += nn.mse_loss(jt.flatten(i2_s_output_), jt.flatten(s2_out)) * 10*23*23*3
    reconstruction_loss += nn.mse_loss(jt.flatten(i1_z_output_), jt.flatten(s1_out)) * 10 * 35 * 35 * 3
    reconstruction_loss += nn.mse_loss(jt.flatten(i2_z_output_), jt.flatten(s2_out)) * 10 * 35 * 35 * 3


    # reconstruction_loss += nn.mse_loss(jt.flatten(i1_z_output_), jt.flatten(i2_z_output_)) * 10000

    rel_z = jt.array(np.maximum(s1_out.numpy(),s2_out.numpy()))

    tms1 = jt.clamp(inputs1 - rel_z, 0, 1)
    tms2 = jt.clamp(inputs2 - rel_z, 0, 1)

    # zzzz = jt.array(np.minimum(tmz1.numpy(),tmz2.numpy()))
    reconstruction_loss += nn.mse_loss(jt.flatten(i1_s_output_), jt.flatten(tms1) )* 10*35*35*3  #10000
    reconstruction_loss += nn.mse_loss(jt.flatten(i2_s_output_), jt.flatten(tms2)) * 10*35*35*3
    # reconstruction_loss += nn.mse_loss(jt.flatten(i1_z_output_), jt.flatten(jt.clamp(inputs1 - s1_out, 0, 1))) * 10000
    # reconstruction_loss += nn.mse_loss(jt.flatten(i2_z_output_), jt.flatten(jt.clamp(inputs2 - s2_out, 0, 1))) * 10000
...
def val2(model1, model2, val_loader1, val_loader2,epoch):
    ...
    for d1,d2 in zip(enumerate(val_loader1), enumerate(val_loader2)):
        ...
        if epoch % 50 == 0:
            checkpoint_path = os.path.join('checkpoints', 'charm')
            checkpoint_name = os.path.join(checkpoint_path, str(epoch) + 'net2.pkl')
            os.makedirs(checkpoint_path, exist_ok=True)
...
def train4(model3, model4, train_loader1, train_loader2, lossfun, optimizer, epoch):
    ...
    for d1, d2 in zip(enumerate(train_loader1), enumerate(train_loader2)):
        ...
        outputs4 = model4(input1, input2,False, 0.01, 1)  # 0.1 0.5 1 1
        z1_fr_net1 = outputs3[4][2]
        z2_fr_net1 = outputs3[4][3]
        loss = lossfun(outputs4, input1, input2, z1_fr_net1,z2_fr_net1)
        optimizer.step(loss)
...
def val3(model2, model3, val_loader1, val_loader2,epoch):
    ...
    for d1, d2 in zip(enumerate(val_loader1), enumerate(val_loader2)):
        ...
        if epoch % 50 == 0:
            checkpoint_path = os.path.join('checkpoints', 'charm')
            checkpoint_name = os.path.join(checkpoint_path, str(epoch) + 'net3.pkl')
            os.makedirs(checkpoint_path, exist_ok=True)
...
def val4(model3, model4, val_loader1, val_loader2,epoch):
    ...
    for d1, d2 in zip(enumerate(val_loader1), enumerate(val_loader2)):
        ...
        if epoch % 50 == 0:
            checkpoint_path = os.path.join('checkpoints', 'charm')
            checkpoint_name = os.path.join(checkpoint_path, str(epoch) + 'net4.pkl')
            os.makedirs(checkpoint_path, exist_ok=True)
...
for i in range(15000):
    ...
    elif i < 2300 and i >= 1500:
        train3(model2, model3, train_loader5, train_loader6, loss3, optimizer3, i)
        val3(model2, model3, val_loader5, val_loader6, i)


    elif i < 3500 and i >= 2300:
        train4(model3, model4, train_loader7, train_loader8, loss4, optimizer4, i)
        val4(model3, model4, val_loader7, val_loader8, i)
