...
import numpy as np
from SU_VAE2 import CVAE
from SU_VAE2 import CVAE2
from SU_VAE2 import CVAE3
from SU_VAE2 import CVAE4
...
train_i1_dir = './data/i1_train'
train_i2_dir = './data/i2_train'
...
val_i1_dir = './data/i1_eval'
val_i2_dir = './data/i2_eval'
...
train_i3_dir = './data/i1_train'
train_i4_dir = './data/i2_train'
...
val_i3_dir = './data/i1_eval'
val_i4_dir = './data/i2_eval'
...
train_i5_dir = './data/i1_train'
train_i6_dir = './data/i2_train'
...
val_i5_dir = './data/i1_eval'
val_i6_dir = './data/i2_eval'
...
train_i7_dir = './data/i1_train'
train_i8_dir = './data/i2_train'
...
val_i7_dir = './data/i1_eval'
val_i8_dir = './data/i2_eval'
...
def loss3(out, inputs1,inputs2,  s1_out, s2_out):
    ...
    reconstruction_loss = nn.mse_loss(jt.flatten(outputs2), jt.flatten(inputs2))* 10*28*28*3  #15000
    reconstruction_loss += nn.mse_loss(jt.flatten(outputs1), jt.flatten(inputs1)) * 10*28*28*3

    reconstruction_loss += nn.mse_loss(jt.flatten(i1_s_output_), jt.flatten(s1_out)) * 10*28*28*3
    reconstruction_loss += nn.mse_loss(jt.flatten(i2_s_output_), jt.flatten(s2_out)) * 10*28*28*3
    ...
    reconstruction_loss += nn.mse_loss(jt.flatten(i1_z_output_), jt.flatten(zzzz) )* 10*28*28*3  #10000
    reconstruction_loss += nn.mse_loss(jt.flatten(i2_z_output_), jt.flatten(zzzz)) * 10*28*28*3
...
def loss4(out, inputs1,inputs2,  s1_out, s2_out):
    ...
    reconstruction_loss += nn.mse_loss(jt.flatten(i1_z_output_), jt.flatten(s1_out)) * 10 * 28 * 28 * 3
    reconstruction_loss += nn.mse_loss(jt.flatten(i2_z_output_), jt.flatten(s2_out)) * 10 * 28 * 28 * 3
    ...
    # zzzz = jt.array(np.minimum(tmz1.numpy(),tmz2.numpy()))
    reconstruction_loss += nn.mse_loss(jt.flatten(i1_s_output_), jt.flatten(tms1) )* 10*28*28*3  #10000
    reconstruction_loss += nn.mse_loss(jt.flatten(i2_s_output_), jt.flatten(tms2)) * 10*28*28*3
    # reconstruction_loss += nn.mse_loss(jt.flatten(i1_z_output_), jt.flatten(jt.clamp(inputs1 - s1_out, 0, 1))) * 10000
    # reconstruction_loss += nn.mse_loss(jt.flatten(i2_z_output_), jt.flatten(jt.clamp(inputs2 - s2_out, 0, 1))) * 10000
...
def val2(model1, model2, val_loader1, val_loader2,epoch):
    ...
    for d1,d2 in zip(enumerate(val_loader1), enumerate(val_loader2)):
        ...
        if epoch % 50 == 0:
            checkpoint_path = os.path.join('checkpoints', 'BA_28')
