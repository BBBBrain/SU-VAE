...
class s2_encoder_f(nn.Module):
    def __init__(self, out_channels=128, intermediate_dim=128, latent_dim=16):
        ...
        # self.s_conv1 = nn.Conv(3, 64, 3, 3, 1)  # no padding
        # self.s_conv2 = nn.Conv(64, out_channels, 3, 2, 1)


        # self.s_conv1 = nn.Conv(3, 16, 3, 2, 1)
        # self.s_conv2 = nn.Conv(16, 64, 3, 2, 1)
        # self.s_conv3 = nn.Conv(64, out_channels, 3, 2, 0)
        #
        ...
        # self.s_h_layer = nn.Linear(out_channels * 3 * 3, intermediate_dim)
        ...
        self.clas = nn.Linear(256, 2)
        self.sof = nn.Sigmoid()
        self.relu = nn.LeakyReLU()
        self.fc1 = nn.Linear(1225*3, 256)
        self.fc21 = nn.Linear(256, 16)
        self.fc22 = nn.Linear(256, 16)

    def execute(self, x):
        # s_h = self.relu(self.s_conv1(x))
        # s_h = self.relu(self.s_conv2(s_h))
        # s_h = self.relu(self.s_conv3(s_h))
...
class decoder_f1(nn.Module):
    def __init__(self, intermediate_dim=256, latent_dim=16):
        super(decoder_f1, self).__init__()
        # build decoder model
        self.rel = nn.LeakyReLU()
        self.sig = nn.Sigmoid()
        self.Li1 = nn.Linear(latent_dim * 2, 256)
        self.Li2 = nn.Linear(256, 1225 * 3)
        self.lab = nn.Linear(1225 * 3, 3)
    ...
    def execute(self, latent_inputs):
        ...
        # print(x.shape)
        x = jt.reshape(x, [x.shape[0], 3, 35, 35])
...
class decoder_f(nn.Module):
    def __init__(self, intermediate_dim=256, latent_dim=16):
        ...
        # self.sig = nn.Sigmoid()
        # self.Li1 = nn.Linear(latent_dim*2, intermediate_dim)
        # self.Li2 = nn.Linear(intermediate_dim, 128*3*3)
        # self.lab = nn.Linear(128*3*3,3)
        # # self.d_conv1 = nn.ConvTranspose(128, 64, 3, stride=3, padding=1)  # 2-> 4
        # # self.d_conv2 = nn.ConvTranspose(64, 32, 3, stride=3, padding=1)  # 4->6
        # # self.d_conv3 = nn.ConvTranspose(32, 3, 3, stride=3, padding=1)  # 6->15
        #
        # self.d_conv1 = nn.ConvTranspose(128, 32, 3, stride=2, output_padding=0 )  # 4-> 7
        # self.d_conv2 = nn.ConvTranspose(32, 8, 3, stride=2, padding=1, output_padding=1)  # 7->14
        # self.d_conv3 = nn.ConvTranspose(8, 3, 3, stride=2, padding=1, output_padding=1)  # 14->28
        # self.bn1 = nn.BatchNorm(32)
        # self.bn2 = nn.BatchNorm(8)
        # # self.d_conv5 = nn.ConvTranspose(8, 3, 2, stride=2, padding=0)  # 28->56
    ...
    def execute(self, latent_inputs):
        ...
        # # print(x.shape)
        # x = jt.reshape(x, [x.shape[0], 128, 3, 3])
        # # print(x.shape)
        # x = self.rel(self.bn1(self.d_conv1(x)))
        # # print(x.shape)
        # x = self.rel(self.bn2(self.d_conv2(x)))
        # x = self.d_conv3(x)
        #
        #
        # # print(x.shape)
        ...
        x = self.rel(self.Li1(latent_inputs*2))
        # print(x.shape)
        x = self.rel(self.Li2(x))
        labs = self.sig(self.lab(x))
        # print(x.shape)
